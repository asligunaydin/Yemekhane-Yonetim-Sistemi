...
<head><base href="">
    <meta charset="utf-8"/>
    <title>Yemekhane Yönetim Sistemi | Yönetim Kurulu</title>
                    ...
                    <div class=" container ">
                        ...
                        <div class="card card-custom">
                            ...
                            <div class="card-body">
                                ...
                                <!--begin: Datatable-->
                                <table class="table table-striped table-light-dark table-bordered table-active" id="kurul_datatable">
                                    <thead>
                                    <tr>
                                        <th title="Field #1">ID</th>
                                        <th title="Field #1">Kurul Üyeliği</th>
                                        <th title="Field #3">Ad</th>
                                        <th title="Field #4">Soyad</th>
                                        <th title="Field #2">Unvan</th>
                                        <th>Düzenle</th>
                                        <th>Sil</th>
                                    </tr>
                                    </thead>
...
<script type="text/javascript">
    ...
    function createDeleteBtn(){
        return '<div class="example-preview">' +
            '<button class="btn btn-sm btn-clean btn-icon uyesil " id="DeleteBtn" type="button" name="uyeSil" data-toggle="modal"  data-target="#modal_delete_kurul" ' +
            ...
            '</a>' +
            '</button>'+
            '<div class="modal fade" id="modal_delete_kurul" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">'+
            ...
            '<button type="button" class="btn btn-danger btn-shadow font-weight-bold " id="ModalUyeSil" data-dismiss="modal">Sil</button>'+
            '</div>'+
            ' </div>'+
            '</div>'+
            '</div>' +
    ...
    //Üye Silme İşlemi Post Başlangıç
    $('#kurul_datatable').on('click','.uyesil',function (){

        var data =$('#kurul_datatable').DataTable().row($(this).parents('tr')).data();
        var deleteID=data['krl_ID']


        $('#modalDeleteID').val(deleteID);

        $('#ModalUyeSil').click(function (e){
            var postDeleteID=$("input[name=modalDeleteID]").val();

            $.ajax({
                url:"KurulSil.php",
                type: "POST",
                data:{'deleteID':postDeleteID,'uyeSilme':true},
                processData:true,
                success:function (data){
                    console.log(data);
                    $('#kurul_datatable').DataTable().ajax.reload();
                    Swal.fire(
                        {
                            position: 'top-right',
                            icon: 'info',
    ...
    //Üye Silme İşlemi Post Bitişi

    function createUpdateBtn(){
        return '<div class="example-preview">' +
            '<button class="btn btn-sm btn-clean btn-icon updateUye" id="updateBtn"  type="button" data-toggle="modal" data-target="#modal_update_kurul" ' +
            '<a href="javascript:;" class="btn btn-sm btn-clean btn-icon " title="Düzenle">' +
            '<span class="svg-icon svg-icon-md">' +
            '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px"  viewBox="0 0 24 24" version="1.1">' +
            '<g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">' +
            ' <rect x="0" y="0"  width="24" height="24"/>' +
            ...
            '</a>' +
            '</button>' +
            '<div class="modal fade" id="modal_update_kurul" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">'+
            ...
            '</button>'+
            '</div>'+
            '<form class="form" style="margin-left: 120px">'+
            '<div class="card-body">'+

            '<div class="form-group row">'+
            '<label class="col-form-label text-right col-lg-3 col-sm-12"></label>'+
            '<div class="col-lg-6">'+
            '<input type="hidden" id="krl_ID" name="krl_ID" class="form-control"placeholder=""/>'+
            '</div>'+
            '</div>'+

            '<div class="form-group row">'+
            '<label class="col-lg-2 col-form-label text-right">Ad:</label>'+
            '<div class="col-lg-4">'+
            '<input class="form-control" name="upd_krl_Ad" id="upd_krl_Ad" type="text" />'+
            '</div>'+
            '</div>'+
            '<div class="form-group row">'+
            '<label class="col-lg-2 col-form-label text-right">Soyad:</label>'+
            '<div class="col-lg-4">'+
            '<input class="form-control" name="upd_krl_Soyad" id="upd_krl_Soyad" type="text" />'+
            '</div>'+
            '</div>'+
            '<div class="form-group row">'+
            '<label class="col-lg-2 col-form-label text-right">Unvan:</label>'+
            '<div class="col-lg-4">'+
            '<input class="form-control" name="upd_krl_Unvan" id="upd_krl_Unvan" type="text" />'+
            '</div>'+
            '</div>'+
            '<div class="form-group row">'+
            '<label class="col-lg-2 col-form-label text-right">Tür:</label>'+
            '<div class="col-lg-4">'+
            '<select class="form-control" id="upd_krl_Tur"  name="upd_krl_Tur">'+
            ...
            '<div class="modal-footer">'+
            '<button type="button" class="btn btn-dark-primary font-weight-bold" data-dismiss="modal">İptal</button>'+
            '<button type="button" class="btn btn-success btn-shadow font-weight-bold" id="uyeGuncelle" data-dismiss="modal">Güncelle</button>'+
            '</div>'+
            ' </div>'+
            '</div>'+
            '</div>' +
    ...
    $('#kurul_datatable').on('click', '.updateUye', function (){

        var eskiVeri=$('#kurul_datatable').DataTable().row($(this).parents('tr')).data();
        var krl_ID=eskiVeri['krl_ID'];
        var krl_Tur=eskiVeri['krl_Tur'];
        var krl_Ad=eskiVeri['krl_Ad'];
        var krl_Soyad=eskiVeri['krl_Soyad'];
        var krl_Unvan=eskiVeri['krl_Unvan'];

        //Modal'a veri gönder
        $('#krl_ID').val(krl_ID);
        $('#upd_krl_Ad').val(krl_Ad);
        $('#upd_krl_Soyad').val(krl_Soyad);
        $('#upd_krl_Unvan').val(krl_Unvan);
        $('#upd_krl_Tur').val(krl_Tur);

        $('#uyeGuncelle').click(function (e){
            var id=$("input[name=krl_ID]").val();
            var ad=$("input[name=upd_krl_Ad]").val();
            var soyad=$("input[name=upd_krl_Soyad]").val();
            var unvan=$("input[name=upd_krl_Unvan]").val();
            var tur=$("select[name=upd_krl_Tur]").val();

            e.preventDefault();
            $.ajax({
                url:"KurulDuzenle.php",
                type:"POST",
                data:{
                    'krl_ID':id,
                    'krl_Ad':ad,
