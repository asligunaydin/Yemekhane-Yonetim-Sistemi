...
<head><base href="">
    <meta charset="utf-8"/>
    <title>Yemekhane Yönetim Sistemi | Kullanıcılar</title>
                    ...
                    <div class=" container ">
                        ...
                        <div class="card card-custom">
                            ...
                            <div class="card-body">
                                ...
                                <!--begin: Datatable-->
                                <table class="table table-striped table-light-dark table-bordered table-active" id="kullanici_datatable">
                                    <thead>
                                    <tr>
                                        <th title="Field #1">Kullanıcı ID</th>
                                        <th title="Field #3">Ad</th>
                                        <th title="Field #2">Soyad</th>
                                        <th title="Field #4">Eposta</th>
                                        <th title="Field #5">Yetkilendirme</th>
                                        <th>Düzenle</th>
                                        <th>Sil</th>
                                    </tr>
                                    </thead>
...
<script type="text/javascript">
    ...
    function createDeleteBtn() {
        return '<div class="example-preview">' +
            '<button class="btn btn-sm btn-clean btn-icon kullanicisil " id="DeleteBtn" type="button" name="kullaniciSil" data-toggle="modal"  data-target="#modal_delete_kullanici" ' +
            ...
            '</a>' +
            '</button>' +
            '<div class="modal fade" id="modal_delete_kullanici" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">'+
            ...
            '<button type="button" class="btn btn-danger btn-shadow font-weight-bold " id="ModalKullaniciSil" data-dismiss="modal">Sil</button>'+
            '</div>'+
            ' </div>'+
            '</div>'+
            '</div>' +
    ...
    //Kullanıcı Silme İşlemi Post Başlangıç
    $('#kullanici_datatable').on('click', '.kullanicisil',function (){

        var data=$('#kullanici_datatable').DataTable().row($(this).parents('tr')).data();
        var deleteID=data['k_ID'];

        $("#modalDeleteID").val(deleteID);

        $('#ModalKullaniciSil').click(function (e){
            var postDeleteID=$("input[name=modalDeleteID]").val();

            $.ajax({
                url:"KullaniciSil.php",
                type:"POST",
                data:{'deleteID':postDeleteID,'kullanicisilme':true},
                processData:true,
                success:function (data){
                    console.log(data);
                    $('#kullanici_datatable').DataTable().ajax.reload();
                    Swal.fire(
                        {
                            position: 'top-right',
                            icon: 'info',
    ...
    //Kullanıcı Silme İşlemi Post Bitiş

    function createUpdateBtn() {
        return '<div class="example-preview">' +
            '<button class="btn btn-sm btn-clean btn-icon updateKullanicilar" id="updateBtn"  type="button" data-toggle="modal" data-target="#modal_update_kullanicilar" ' +
            ...
            '</a>' +
            '</button>' +
            '<div class="modal fade" id="modal_update_kullanicilar" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">'+
            ...
            '<div class="col-lg-6">'+
            '<input type="hidden" id="k_id" name="k_id" class="form-control"placeholder=""/>'+
            '</div>'+
            '</div>'+
            ...
            '<div class="col-lg-6">'+
            '<input type="text" id="k_ad" name="k_ad" class="form-control"placeholder=""/>'+
            '</div>'+
            '</div>'+
            ...
            '<div class="col-lg-6">'+
            '<input type="text" id="k_soyad" name="k_soyad" class="form-control" placeholder=""/>'+
            '</div>'+
            '</div>'+
            ...
            '<div class="col-lg-6">'+
            '<input type="text" id="k_mail" name="k_mail" class="form-control" placeholder=""/>'+
            '</div>'+
            '</div>'+
            ...
            '<select class="form-control " id="yetki_turu"  name="yetki_turu">'+
            ...
            '<div class="modal-footer">'+
            '<button type="button" class="btn btn-dark-primary font-weight-bold" data-dismiss="modal">İptal</button>'+
            '<button type="button" class="btn btn-success btn-shadow font-weight-bold" id="kullaniciGuncelle" data-dismiss="modal">Güncelle</button>'+
            '</div>'+
            ' </div>'+
            '</div>'+
            '</div>' +
    ...
    $('#kullanici_datatable').on('click','.updateKullanicilar',function (){

        var eskiVeri=$('#kullanici_datatable').DataTable().row($(this).parents('tr')).data();
        var k_id=eskiVeri['k_ID'];
        var k_ad=eskiVeri['k_Ad'];
        var k_soyad=eskiVeri['k_Soyad'];
        var k_mail=eskiVeri['k_Mail'];
        var yetki_turu=eskiVeri['yetki_turu'];
        ...
        $("#k_soyad").val(k_soyad);
        $("#k_mail").val(k_mail);
        $("#yetki_turu").val(yetki_turu);

        $('#kullaniciGuncelle').click(function (e){

            var kllnc_id=$("input[name=k_id]").val();
            var kllnc_ad=$("input[name=k_ad]").val();
            var kllnc_soyad=$("input[name=k_soyad]").val();
            var kllnc_mail=$("input[name=k_mail]").val();
            var kllnc_yetki=$("select[name=yetki_turu]").val();

            e.preventDefault();
            $.ajax({
                url:"KullaniciDuzenle.php",
                type:"POST",
                data:{
                    'k_id':kllnc_id,
                    'k_ad':kllnc_ad,
