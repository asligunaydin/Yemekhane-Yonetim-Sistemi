...
<head><base href="">
    <meta charset="utf-8"/>
    <title>Yemekhane Yönetim Sistemi | İstek ve Öneri</title>
                    ...
                    <div class=" container ">
                        ...
                        <div class="card card-custom">
                            ...
                            <div class="card-body">
                                ...
                                <!--begin: Datatable-->
                                <table class="table table-striped table-light-dark table-bordered table-active"  id="oneriler_datatable">
                                    <thead>
                                    <tr>
                                        <th title="Field #1">ID</th>
                                        <th title="Field #1">Tarih</th>
                                        <th title="Field #2">Konu</th>
                                        <th title="Field #3">İleti</th>
                                        <th title="Field #4">Ad</th>
                                        <th title="Field #5">Soyad</th>
                                        <th title="Field #6">Eposta</th>
                                        <th>Görüntüle</th>
                                        <th>Sil</th>
                                    </tr>
                                    </thead>
...
<script type="text/javascript">
    ...
    function createDeleteBtn() {
        return '<div class="example-preview">' +
            '<button class="btn btn-sm btn-clean btn-icon onerisil " id="DeleteBtn" type="button" name="kullaniciSil" data-toggle="modal"  data-target="#modal_delete_oneri" ' +
            ...
            '</a>' +
            '</button>'+
            '<div class="modal fade" id="modal_delete_oneri" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">'+
            ...
            '<button type="button" class="btn btn-danger btn-shadow font-weight-bold " id="ModalOneriSil" data-dismiss="modal">Sil</button>'+
            '</div>'+
            ' </div>'+
            '</div>'+
            '</div>' +
    ...
    //Öneri Silme İşlemi Post Başlangıç
    $('#oneriler_datatable').on('click', '.onerisil',function (){

        var data=$('#oneriler_datatable').DataTable().row($(this).parents('tr')).data();
        var deleteID=data['onr_ID'];

        $("#modalDeleteID").val(deleteID);

        $('#ModalOneriSil').click(function (e){
            var postDeleteID=$("input[name=modalDeleteID]").val();

            $.ajax({
                url:"IstekOneriSil.php",
                type:"POST",
                data:{'deleteID':postDeleteID,'onerisilme':true},
                processData:true,
                success:function (data){
                    $('#oneriler_datatable').DataTable().ajax.reload();
                    Swal.fire(
                        {
                            position: 'top-right',
                            icon: 'info',
    ...
    //Öneri Silme İşlemi Post Bitiş



    function createUpdateBtn() {
        return'<div class="example-preview">' +
            '<button class="btn btn-sm btn-clean btn-icon updateOneriler" id="updateBtn"  type="button" data-toggle="modal" data-target="#modal_update_oneriler" ' +
            ...
            '</a>' +
            '</button>'+
            '<div class="modal fade" id="modal_update_oneriler" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">'+
            '<div class="modal-dialog modal-dialog-centered modal-lg" role="document">'+
            '<div class="modal-content">'+
            '<div class="modal-header">'+
            '<h5 class="modal-title" id="exampleModalLabel">İstek / Öneri Detayı</h5>'+
            '<button type="button" class="close" data-dismiss="modal" aria-label="Close">'+
            '<i aria-hidden="true" class="ki ki-close"></i>'+
            '</button>'+
            '</div>'+
            ...
            '<label class="col-form-label text-right col-lg-3 col-sm-12">Ad ve Soyad:</label>'+
            '<div class="col-lg-4">'+
            '<input type="text" id="ad" name="ad" class="form-control" disabled/>'+
            '</div>'+
            '</div>'+

            '<div class="form-group row">'+
            '<label class="col-form-label text-right col-lg-3 col-sm-12">Konu:</label>'+
            '<div class="col-lg-4">'+
            '<input type="text" id="konu" name="konu" class="form-control" disabled/>'+
            '</div>'+
            '</div>'+

            '<div class="form-group row">'+
            '<label class="col-form-label text-right col-lg-3 col-sm-12">Mesaj:</label>'+
            '<div class="col-lg-4 col-md-9 col-sm-12">'+
            '<textarea class="form-control" id="ileti" name="ileti" placeholder="" rows="6" disabled></textarea>'+
            ...
            '<div class="modal-footer">'+
            '<button type="button" class="btn btn-danger btn-shadow font-weight-bold" data-dismiss="modal">Kapat</button>'+
            '</div>'+
            ' </div>'+
            '</div>'+
            '</div>' +
    ...
    $('#oneriler_datatable').on('click','.updateOneriler',function (){

        var veri =$('#oneriler_datatable').DataTable().row($(this).parents('tr')).data();
        var ad=veri['ad'];
        var soyad=veri['soyad'];
        var konu=veri['konu'];
        var ileti=veri['ileti'];


        //modala veri gönderme
