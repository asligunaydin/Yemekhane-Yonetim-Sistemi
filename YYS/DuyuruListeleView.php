...
<head><base href="">
    <meta charset="utf-8"/>
    <title>Yemekhane Yönetim Sistemi | Duyurular</title>
                    ...
                    <div class=" container ">
                        ...
                        <div class="card card-custom">
                            ...
                            <div class="card-body">
                                ...
                                <!--begin: Datatable-->
                                <table class="table table-striped table-light-dark table-bordered table-active" id="duyurular_datatable">
                                    <thead>
                                    <tr>
                                        <th title="Field #1">Duyuru ID</th>
                                        <th title="Field #3">Başlık</th>
                                        <th title="Field #4">Metin</th>
                                        <th title="Field #2">Yayın Tarih</th>
...
<script type="text/javascript">
    ...
    function createDeleteBtn() {
        ...
        return  '<div class="example-preview">' +
                    '<button class="btn btn-sm btn-clean btn-icon DeleteDuyuru" id="DeleteBtn" type="button" name="duyuruSil" data-toggle="modal" data-target="#modal_delete_duyuru" ' +
                    ...
                    '</a>' +
                '</button>'+
        '<div class="modal fade" id="modal_delete_duyuru" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">'+
        ...
        '<button type="button" class="btn btn-danger btn-shadow font-weight-bold " id="ModalDuyuruSil" data-dismiss="modal">Sil</button>'+
        '</div>'+
        ' </div>'+
        '</div>'+
        '</div>' +
    ...
    $('#duyurular_datatable').on('click', '.DeleteDuyuru', function(){

        var data =$('#duyurular_datatable').DataTable().row($(this).parents('tr')).data();
        var deleteID = data['duyuru_ID'];

        $("#modalDeleteID").val(deleteID);

        $('#ModalDuyuruSil').click(function(e){
            ...
            $.ajax({
                ...
                success:function (data){
                    ...
                    console.log(data);
                    $('#duyurular_datatable').DataTable().ajax.reload();
                    Swal.fire({
                        position:'top-right',
                        icon:'info',
                        title: data,
    ...
    function createUpdateBtn() {
        return '<div class="example-preview">' +
            '<button class="btn btn-sm btn-clean btn-icon UpdateDuyuru" id="updateBtn"  type="button" data-toggle="modal" data-target="#modal_update_duyuru" ' +
            ...
            '</a>' +
            '</button>'+
            '<div class="modal fade" id="modal_update_duyuru" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">'+
            ...
            '<div class="col-lg-4">'+
            '<input type="hidden" id="duyuru_ID" name="duyuru_ID" class="form-control" placeholder=""/>'+
            ...
            '<div class="col-lg-4">'+
            '<input type="text" id="duyuru_Baslik" name="duyuru_Baslik" class="form-control" placeholder=""/>'+
            ...
            '<input type="text" id="duyuru_Tarih" name="duyuru_Tarih" class="form-control form-control-solid datetimepicker-input" placeholder="" data-target="#kt_datetimepicker_3"/>'+
            '<div class="input-group-append"  name="duyuru_Tarih" data-target="#kt_datetimepicker_3" data-toggle="datetimepicker">'+
            ...
            '<div class="col-lg-4 col-md-9 col-sm-12">'+
            '<textarea class="form-control" id="duyuru_Metin" name="duyuru_Metin" placeholder="" rows="6"></textarea>'+
            ...
            '<div class="modal-footer">'+
            '<button type="button" class="btn btn-dark-primary font-weight-bold" data-dismiss="modal">İptal</button>'+
            '<button type="button" class="btn btn-success btn-shadow font-weight-bold" id="duyuruGuncelle" data-dismiss="modal">Kaydet</button>'+
            '</div>'+
            ' </div>'+
            '</div>'+
            '</div>' +
    ...
    //Duyuru Güncelleme Veri Post Başlangıcı
    $('#duyurular_datatable').on('click','.UpdateDuyuru', function (){

        var eskiVeri=$('#duyurular_datatable').DataTable().row($(this).parents('tr')).data();
        var d_id=eskiVeri['duyuru_ID'];
        var d_baslik=eskiVeri['duyuru_Baslik'];
        var d_tarih=eskiVeri['duyuru_Tarih'];
        var d_metin=eskiVeri['duyuru_Metin'];


        //verileri modala yazdır
        $('#duyuru_ID').val(d_id);
        $('#duyuru_Baslik').val(d_baslik);
        $('#duyuru_Tarih').val(d_tarih);
        $('#duyuru_Metin').val(d_metin);

        $('#duyuruGuncelle').click(function (e){

            var duyuru_id=$("input[name=duyuru_ID]").val();
    ...
    //Duyuru Güncelleme Veri Post Bitişi
